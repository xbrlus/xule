...
def build_xule_rules(template_tree, template_file_name):
    ...
    for xule_expression in template_tree.findall('//xule:expression', _XULE_NAMESPACE_MAP):
        ...
        if named_rule is None:
            ...
            if xule_expression.get('type') == 'fact':
                ...
                substitutions[rule_name].append({'part': None, 'replacement-node': replacement_node, 'result-text-index': 0})
    ...
    for named_rule, part_list in named_rules.items():
        ...
        for part, expression in part_list:
            ...
            if part is None:
                ...
                if expression.get("type") == 'fact':
                    ...
                    result_parts.append('(({})#rv-{}).string'.format(expression.text.strip(), part))
                    rule_focus.append('$rv-{}'.format(part))
                    substitutions[rule_name].append({'name': named_rule, 'part': part, 'replacement-node': replacement_node, 'result-focus-index': next_focus_number})
                    next_focus_number += 1
                else:
                    result_parts.append('({}).string'.format(expression.text.strip()))
                    substitutions[rule_name].append({'name': named_rule, 'part': part, 'replacement-node': replacement_node, 'result-text-index': next_text_number})
        ...
        result_text = ', '.join(["{}".format(expression) for expression in result_parts])
        ...
        for rule_result in rule_results[rule_name]:
            ...
            for sub in subs:
                ...
                if rule_name in rule_results:
                    ...
                    xule_expression = sub['replacement-node'].find('xule:expression', _XULE_NAMESPACE_MAP)
                    if xule_expression is not None and xule_expression.get('type') == 'fact':
                        fact_object_index = rule_result.refs[sub['result-focus-index']]['objectId']
                        model_fact = modelXbrl.modelObject(fact_object_index)

                        text_content, inline_format_clark = format_fact(xule_expression, model_fact)
                    ...
                    text_version = '<span class="sub-value sub-no-replacement"/>'

                
                span = etree.fromstring(text_version)
