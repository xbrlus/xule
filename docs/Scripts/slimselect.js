        ...
        if (this.main.config.isMultiple) {
            ...
            for (var _i = 0, options_2 = options; _i < options_2.length; _i++) {
                ...
                if (o.selected) {
                    ...
                    var fullvalue = o.value;

                    var strNdx = fullvalue.indexOf(".sort(");

                    if (strNdx > 0 )
                        fullvalue = fullvalue.substring(0, strNdx);

                    var newOption = this.getObjectFromData(fullvalue, 'value');
...
var Config = (function () {
    function Config(info) {
        ...
        this.isSortable = (info.isSortable === false ? false : true);
        if (info.showContent) {
...
var Slim = (function () {
    ...
    Slim.prototype.valueDiv = function (optionObj) {
        ...
        value.id = "d" + optionObj.id;


        value.ondrop = function (ev) {
            ev.preventDefault();
            var parentText = ev.target.innerText;
            var parent = ev.target.parentNode;
            var child = $("#" + ev.dataTransfer.getData("dragId"));
            var childText = child[0].childNodes[0].innerText;


            if ((child[0].classList[0] == 'ss-value' && parent.classList[0] == 'ss-value')   ){

                child.insertBefore(parent);
                //var inputChild = document.getElementById(childText);
                //var inputParent = document.getElementById(parentText);
                var inputChild = $("#" + childText);
                var inputParent = $("#" + parentText);
                var ele = _this.main.select.element;

                //$('option["' + childText + '"]').insertBefore('option["' + parentText + '"]');
                //$('option[id="' + childText + '"]').insertBefore($('option[id="' + parentText + '"]'));
                inputChild.insertBefore(inputParent);
        ...
        value.title = "Drag and drop to order columns";

        if (_this.main.config.isSortable) {

            value.title = ("Drag and drop to order columns; \nuse ↑↓ to sort data (by order)");

            var sortAscSpan = document.createElement('span');
            var sortDescSpan = document.createElement('span');
            sortAscSpan.classList.add(this.main.config.valueDelete);
            sortAscSpan.innerHTML = '↑';
            sortAscSpan.onclick = function (e) {

                var optVal = optionObj.text;
                

                if (sortAscSpan.classList.contains("selected")) {
                    sortAscSpan.classList.remove("selected");
                    optVal = optionObj.text;
                }
                else {
                    //$("." + _this.main.config.id + " span").removeClass("selected");
                    sortDescSpan.classList.remove("selected");
                    sortAscSpan.classList.add("selected");
                    optVal = optionObj.text + ".sort(ASC)";
                }

                
                
                var selected = _this.main.select.element.selectedOptions;

                for (var i = 0; i < selected.length; i++) {
                    if (selected[i].id === optionObj.text) {
                        selected[i].value = optVal;
                        break;
                    }
                    //else {
                    //    selected[i].value = selected[i].id;
                    //}
                }
            ...
            value.appendChild(sortAscSpan);


            sortDescSpan.classList.add(this.main.config.valueDelete);
            sortDescSpan.innerHTML = '↓';
            sortDescSpan.onclick = function (e) {
                var optVal = optionObj.text;


                if (sortDescSpan.classList.contains("selected")) {
                    sortDescSpan.classList.remove("selected");
                    optVal = optionObj.text;
                }
                else {
                    ...
                    //$("." + _this.main.config.id + " span").removeClass("selected");
                    sortAscSpan.classList.remove("selected");
                    sortDescSpan.classList.add("selected");
                    optVal = optionObj.text + ".sort(DESC)";
            ...
            value.appendChild(sortDescSpan);
        }
